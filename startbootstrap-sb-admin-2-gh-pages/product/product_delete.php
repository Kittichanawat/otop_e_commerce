...
if (isset($_GET['prd_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
        ...
        if (mysqli_query($proj_connect, $delete_img_sql)) {
            // Delete the main image path from the product table
            $delete_main_image_sql = "UPDATE product SET prd_img = '' WHERE prd_id = $prd_id";

            if (mysqli_query($proj_connect, $delete_main_image_sql)) {
                // Delete the product data
                $delete_sql = "DELETE FROM product WHERE prd_id = $prd_id";

                if (mysqli_query($proj_connect, $delete_sql)) {
                    header("Location: ../product/"); // Redirect to product list page after successful delete
                    exit();
                } else {
                    echo "Error deleting product: " . mysqli_error($proj_connect);
                }
            } else {
                echo "Error deleting main image path: " . mysqli_error($proj_connect);
            }
        } else {
