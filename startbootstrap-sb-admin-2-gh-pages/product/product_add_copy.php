...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["main_image"]["tmp_name"])) {
        ...
        $imageType = exif_imagetype($mainImgTmpName);
        ...
        if ($imageType !== false) {
            if ($imageType == IMAGETYPE_JPEG || $imageType == IMAGETYPE_PNG || $imageType == IMAGETYPE_GIF) {
                ...
                echo "ขออภัย, สามารถอัพโหลดได้เฉพาะไฟล์ JPG, PNG และ GIF เท่านั้น.";
                exit;
            }
        } else {
    ...
    if ($proj_connect->query($sql) === TRUE) {
        ...
        if (!empty($_FILES["additional_images"]["tmp_name"])) {
            ...
            foreach ($_FILES["additional_images"]["tmp_name"] as $key => $tmp_name) {
                ...
                // ตรวจสอบว่าไฟล์เป็นรูปภาพ
                $imageType = exif_imagetype($img_tmp_name);
                ...
                if ($imageType !== false) {
                    ...
                    echo "ไฟล์ไม่ใช่รูปภาพ: " . $img_name;
                }
            }
        }
      ...
      <?php include('../web_stuc/side_bar.php');?>
        ...
        <form action="" method="post" enctype="multipart/form-data" onSubmit="return chkdata(this);">
            ...
            <div class="mb-3">
               ...
               <input type="file" name="main_image" id="main_image" class="form-control" accept="image/png, image/gif, image/jpeg" />
