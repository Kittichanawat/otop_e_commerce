...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $new_bank_number = $_POST["bank_number"];
    $new_bank_show = $_POST["bank_show"];
    ...
    if (isset($_FILES["new_image"]) && $_FILES["new_image"]["name"]) {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($new_image["tmp_name"], $targetFile)) {
                ...
                $update_sql = "UPDATE banking SET bank_name = '$new_bank_name', acc_name = '$new_acc_name',  bank_number = '$new_bank_number',bank_img = '$targetFile', bank_show = '$new_bank_show' WHERE id = '$id'";
        ...
        $update_sql = "UPDATE banking SETbank_name = '$new_bank_name', acc_name = '$new_acc_name',  bank_number = '$new_bank_number', bank_show = '$new_bank_show' WHERE id = '$id'";
        if ($proj_connect->query($update_sql) === TRUE) {
            $edit_success = true;
        } else {
            echo "ข้อผิดพลาดในการแก้ไขหน้า: " . $proj_connect->error;
...
<body id="page-top">
    ...
    <form action="" method="post"   enctype="multipart/form-data" onSubmit="return chkdata(this);">
        <div class="mb-3">
            <label for="bank_name" class="form-label">ชื่อหน้า:</label>
            <input type="text" name="bank_name" id="bank_name" class="form-control" value="<?php echo $bank_name; ?>">
        </div>
        <div class="mb-3">
            <label for="acc_name" class="form-label">รายละเอียดหน้า:</label>
            <textarea name="acc_name" id="acc_name" rows="4" class="form-control" ><?php echo $acc_name; ?></textarea>
        </div>
        <div class="mb-3">
            <label for="bank_number" class="form-label">ลิงก์ข้อมูลเพิ่มเติม:</label>
            <textarea type="text" name="bank_number" id="bank_number" class="form-control" value=""><?php echo $bank_number; ?></textarea>
        </div>
        ...
        <div class="row">
            ...
            $query_existing_images = "SELECT * FROM banking WHERE id = '$id'";
            ...
            if ($result_existing_images->num_rows > 0) {
                ...
                $existing_image_path = $row_existing_image['bank_img'];
        ...
        <div class="mb-3">
                <label for="bank_show" class="form-label">แสดงหน้าต่าง:</label>
                <select id="bank_show" name="bank_show" class="form-control " required>
                    <option value="" required>..กรุณาเลือก..</option>
                    <option value="0" <?php if ($bank_show == 0) echo "selected"; ?> required>ไม่แสดง</option>
                    <option value="1" <?php if ($bank_show == 1) echo "selected"; ?> required>แสดง</option>
                </select>
            </div>
      
        <div class="mb-3">
            <input type="submit" name="submit" value="บันทึกการแก้ไข" class="btn btn-primary">
            <a href="../banking/" class="btn btn-secondary">cancel</a>
        </div>
    </form>
</div>
