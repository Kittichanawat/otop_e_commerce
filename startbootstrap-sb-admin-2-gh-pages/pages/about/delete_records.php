...
if ($jsonData) {
    ...
    if ($requestData && isset($requestData['aIds']) && is_array($requestData['aIds'])) {
        $deleteIDs = implode(',', $requestData['aIds']); // Get the IDs of selected records as a comma-separated string.
        $sql = "DELETE FROM about WHERE a_id IN ($deleteIDs)";
