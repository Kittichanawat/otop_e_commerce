...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO history (h_title, h_detail, h_img, h_link,h_show) 
            VALUES ('$h_title', '$h_detail', '$mainImgPath', '$h_link','$h_show')";
...
<body id="page-top">
    ...
    <form action="" method="post" enctype="multipart/form-data"  onSubmit="return chkdata(this);">
        <div class="mb-3">
            <label for="h_title" class="form-label">หัวข้อ:</label>
            <input type="text" name="h_title" id="h_title" class="form-control">
        </div>
        <div class="mb-3">
            <label for="h_detail" class="form-label">รายละเอียด:</label>
            <textarea name="h_detail" id="h_detail" rows="4" class="form-control"></textarea>
        </div>
        ...
        <div class="mb-3">
            <label for="h_link" class="form-label">ลิงค์:</label>
            <input type="text" name="h_link" id="h_link" class="form-control">
        </div>
        <div class="mb-3">
                <label for="h_show" class="form-label">แสดงหน้าต่างๆ:</label>
                <select id="h_show" name="h_show" class="form-control">
...
function chkdata(form) {
    ...
    if (form.h_show.value === "") {
        errorMessage += " แสดงประวัติ";
        showError = 1;
    }
