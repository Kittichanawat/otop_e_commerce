...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // หาค่าสูงสุดของ order_column และเพิ่มด้วย 1
    $orderQuery = "SELECT MAX(order_column) as maxOrder FROM banner";
    $orderResult = $proj_connect->query($orderQuery);
    if ($orderResult->num_rows > 0) {
        $orderRow = $orderResult->fetch_assoc();
        $nextOrder = $orderRow['maxOrder'] + 1;
    } else {
        $nextOrder = 1; // ถ้าไม่มีข้อมูลใดๆ เริ่มที่ 1
    }

    $sql = "INSERT INTO banner (title, detail, img, link, pages_show, order_column) 
            VALUES ('$title', '$detail', '$mainImgPath', '$link', '$pages_show', '$nextOrder')";
