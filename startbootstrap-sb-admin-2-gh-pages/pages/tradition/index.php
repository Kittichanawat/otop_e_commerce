...
$sql_script = "SELECT * FROM tradition";
...
?>
    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <h1 class=" text-center ">หน้าจัดการประเพณี</h1>
                <a href="tradition_add.php" class="text-left mb-3 ml-3 btn btn-primary" >เพิ่มประเพณี </a>
    ...
    $stmt = $conn->query("SELECT * FROM tradition");
    $stmt->execute();
    $members = $stmt->fetchAll();

    // Initialize a variable to keep track of whether there are any records
    $hasRecords = false;
    ...
    foreach ($members as $member) {
        ...
        ?>
        <tr>
            <td><input type="checkbox" name="delete[]" value="<?php echo $member['t_id']; ?>"></td>
            <td><?php echo $member['t_id'] ?></td>
            <td><?php echo $member['t_name'] ?></td>
            <td class="text-truncate overflow-hidden" style="max-width: 200px;"><?php echo $member['t_detail'] ?></td>
            <td><img src="<?php echo $member['t_img'] ?>" alt="" style="max-width: 100px; max-height: 100px;"></td>
            <td><?php echo $member['t_show'] == 1 ? 'Enable' : 'Disable'; ?></td>
            <td><?php echo $member['t_reccom'] == 1 ? 'Enable' : 'Disable'; ?></td>
            <td><a href="tradition_edit.php?t_id=<?php echo $member['t_id'] ?>" class="btn btn-warning">แก้ไข</a></td>
            <td><a onclick="confirmDelete(<?php echo $member['t_id']; ?>, '<?php echo $member['t_name']; ?>')" class="btn btn-danger">ลบ</a></td>
        </tr>
    <?php
    }
    ?>
...
<?php if ($hasRecords) : ?>
    <a onclick="confirmDeleteSelect(<?php echo $member['t_id']; ?>)" name="delete_records" class="btn btn-danger">Delete Selected</a>
<?php endif; ?>
    ...
    function confirmDelete(t_id, t_name) {
        Swal.fire({
            ...
            text: `คุณต้องการลบข้อมูล "${t_name}" ใช่หรือไม่?`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                fetch(`tradition_delete.php?t_id=${t_id}`)
...
<script>
                ...
                function deleteSelectedItems(checkboxes) {
                    const tIds = Array.from(checkboxes).map(checkbox => checkbox.value);
