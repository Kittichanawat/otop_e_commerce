...
// Check if the image ID and t_id are provided in the query string
if (isset($_GET['img_id']) && isset($_GET['t_id'])) {
    $img_id = $_GET['img_id'];
    $t_id = $_GET['t_id']; // Get the t_id from the query string

    // Get the image path from the database
    $query_image_path = "SELECT img FROM tradition_img WHERE img_id = '$img_id'";
    ...
    if ($result_image_path->num_rows > 0) {
        ...
        $delete_query = "DELETE FROM tradition_img WHERE img_id = '$img_id'";
    ...
    // Set an error message in the response
    $response['success'] = false;
    $response['message'] = "Image ID or t_id not provided.";
}
