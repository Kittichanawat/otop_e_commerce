    ...
    if (isset($_FILES["new_image"]) && $_FILES["new_image"]["name"]) {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($new_image["tmp_name"], $targetFile)) {
                ...
                $update_sql = "UPDATE contact SET title = '$new_title', detail = '$new_detail', img = '$targetFile', link = '$new_link', pages_show = '$new_pages_show' WHERE id = '$id'";
        ...
        $update_sql = "UPDATE contact SET title = '$new_title', detail = '$new_detail', link = '$new_link', pages_show = '$new_pages_show' WHERE id = '$id'";
        if ($proj_connect->query($update_sql) === TRUE) {
            $edit_success = true;
        } else {
            echo "ข้อผิดพลาดในการแก้ไขหน้า: " . $proj_connect->error;
...
<body id="page-top">
    ...
    <form action="" method="post"   enctype="multipart/form-data" onSubmit="return chkdata(this);">
        ...
        <div class="row">
            ...
            $query_existing_images = "SELECT * FROM contact WHERE id = '$id'";
