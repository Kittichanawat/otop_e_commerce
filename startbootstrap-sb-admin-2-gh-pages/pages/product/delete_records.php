...
if ($jsonData) {
    ...
    if ($requestData && isset($requestData['prdIds']) && is_array($requestData['prdIds'])) {
        $deleteIDs = implode(',', $requestData['prdIds']); // Get the IDs of selected records as a comma-separated string.
        $sql = "DELETE FROM product WHERE prd_id IN ($deleteIDs)";
