...
// Check if the tourist ID is provided in the URL
if (isset($_GET['p_id'])) {
    $p_id = $_GET['p_id'];

    // Get the folder name based on the p_id
    $folderName = "place_additional_img/" . $p_id;

    // Delete the folder and its contents
    deleteDirectory($folderName);

    // Delete records from the tourist_img table
    $delete_img_sql = "DELETE FROM tourist_img WHERE p_id = $p_id";

    if (mysqli_query($proj_connect, $delete_img_sql)) {
        // Delete the tourist data
        ...
        if (mysqli_query($proj_connect, $delete_sql)) {
            ...
            echo json_encode(['success' => false, 'message' => 'Error deleting tourist: ' . mysqli_error($proj_connect)]);
        }
    } else {
        echo json_encode(['success' => false, 'message' => 'Error deleting tourist images: ' . mysqli_error($proj_connect)]);
    }
} else {
    // Handle the case where p_id is not provided in the URL
    echo json_encode(['success' => false, 'message' => 'Tourist ID not provided.']);
    exit;
}
?>
