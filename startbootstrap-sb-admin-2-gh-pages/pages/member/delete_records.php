...
if ($jsonData) {
    ...
    if ($requestData && isset($requestData['mmbIds']) && is_array($requestData['mmbIds'])) {
        $deleteIDs = implode(',', $requestData['mmbIds']); // Get the IDs of selected records as a comma-separated string.
        $sql = "DELETE FROM member WHERE mmb_id IN ($deleteIDs)";
