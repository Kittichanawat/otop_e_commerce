...
<script>
    $(document).ready(function () {
        // เมื่อคลิกปุ่ม "ซ่อน" หรือ "แสดง"
        $('button.toggleBtn').on('click', function () {
            var mmb_id = $(this).data('mmb-id');
            var action = $(this).data('action');

            // ส่ง Ajax request ไปยังไฟล์ PHP เพื่ออัพเดทค่า mmb_show
            $.ajax({
                type: 'POST',
                url: 'update_member_status.php', // เปลี่ยนเป็นชื่อไฟล์ PHP ที่คุณสร้าง
                data: { mmb_id: mmb_id, action: action },
                success: function (response) {
                    // หลังจากอัพเดทเสร็จแล้ว
                    // คุณสามารถทำอะไรก็ตามที่คุณต้องการ, เช่น อัพเดท UI, แสดงข้อความ, เปลี่ยนสีปุ่ม, ฯลฯ
                    // ตัวอย่าง: แก้ไข HTML ของปุ่ม
                    if (action === 'hide') {
                        $('button.toggleBtn[data-mmb-id=' + mmb_id + ']').text('แสดง');
                    } else {
                        $('button.toggleBtn[data-mmb-id=' + mmb_id + ']').text('ซ่อน');
                    }
                },
                error: function () {
